                        ...
                        function () {
                            ...
                            t.on('kt-datatable--on-layout-updated', function () {
                                $('.btn-view-detail').click(function () {
                                    ...
                                    getDetailTransferMoneyTransactionById(transactionId)
                                });
                            });

                            function getDetailTransferMoneyTransactionById(id)
                            {
                                if ($('#tranferMoneyTransactionId' + id).length === 0) {
                                    $.ajax({
                                        type: "get",
                                        url: "/transfer-money-transactions/detail/" + id,
                                        success: function (response) {
                                            $('body').append(response);
                                            $('#tranferMoneyTransactionId' + id).modal('show');
                                        }
                                    });
                                } else {
                                    $('#tranferMoneyTransactionId' + id).modal('show');
