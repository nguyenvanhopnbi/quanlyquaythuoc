...
var KTDashboard = function() {
    ...
    function getFlashReport(){
        $.ajax({
            type: "GET",
            url: "/gate-money-dashboard/get-flash-report",
            data: {
                query : {
                    partner_code: $("#partner_code_flash").val(),
                    'startDate': $('#time-from-flash').val(), 
                    'endDate': $('#time-to-flash').val()
                }
            },
            success: function (response) {
                // $('#count_transaction_flash').text(addCommas(response.data.new_data.ttcard));
                // $('#sum_transaction_flash').text(addCommas(response.data.new_data.ttamount));
                // console.log(response.data.old_data.ttamount);
                // $('#count_transaction_before_flash').text(addCommas(response.data.old_data.ttcard));
                // $('#sum_transaction_before_flash').text(addCommas(response.data.old_data.ttamount));
    ...
    function getFlashReport2(){
        $.ajax({
            type: "GET",
            url: "/gate-money-dashboard/get-flash-report2",
            data: {
                query : {
                    vendor_code: $("#vendor_code").val(),
                    bank_code: $("#bank_code").val(),
                    'startDate': $('#time-from-flash2').val(),
                    'endDate': $('#time-to-flash2').val(),
                    partner_code: $('#partner_code2').val(),
                }
            },
            success: function (response) {
                // $('#count_transaction_flash').text(addCommas(response.data.new_data.ttcard));
                ...
                // $('#count_transaction_before_flash').text(addCommas(response.data.old_data.ttcard));
                // $('#sum_transaction_before_flash').text(addCommas(response.data.old_data.ttamount));
                $('#table_flash_report_holder2').html(response);
            }
        });
    }
    ...
    $('.btn-get-flash-chart2').click(function(e){
        e.preventDefault();
        getFlashReport2();
    })
    ...
    var buildChart = function() {
        ...
        $.ajax({
            type: "GET",
            url: "/gate-money-dashboard/get-chart-transaction",
            data: {data: dataSearchChart},
            success: function (response) {
                latestUpdates(response.data);

                var formatter = new Intl.NumberFormat('en');
                var sumFormat = formatter.format(response.data.sum) + " VNĐ";

                $('#count_transaction').text(response.data.count)
                $('#sum_transaction').text(sumFormat)
            }
        });
    }
    ...
    $("#time-from-flash2").change(function(){
        dataSearchChart.condition = 'select';
        dataSearchChart.startDate = $(this).val();
    })

    $("#time-to-flash2").change(function(){
        dataSearchChart.condition = 'select';
        dataSearchChart.endDate = $(this).val();
    })
    ...
    function buildDatePicker()
    {
        ...
        $("#time-from-flash2").datepicker({
            rtl: KTUtil.isRTL(),
            clearBtn: !0,
            todayHighlight: !0,
            orientation: "bottom left",
        });
        $("#time-to-flash2").datepicker({
            rtl: KTUtil.isRTL(),
            clearBtn: !0,
            todayHighlight: !0,
            orientation: "bottom left",
    ...
    function exportTransaction()
    {
        $('.btn-export-flash-chart').click(function(e){
            ...
            $.ajax({
                type: "POST",
                url: "/gate-money-dashboard/export",
                data: {
                    query : {
                        vendor_code: $("#vendor_code").val(),
                        bank_code: $("#bank_code").val(),
                ...
                success: function (response) {
                    if (response.code == 200) {
                        location.href = '/gate-money-dashboard/download?file='+response.path;
...
jQuery(document).ready(function() {
    ...
    function formatPartner(repo) {
        ...
        var markup = "<div class='select2-result-repository clearfix'>" +
            "<div class='select2-result-repository__meta'>" +
            "<div class='select2-result-repository__title'>" + repo.vendor_code + "(" + repo.vendor_name +")"  + "</div>";
        return markup;
    }

    function formatPartnerSelection(repo) {
        if(typeof repo.vendor_code !== 'undefined'){
            return repo.vendor_code + "(" + repo.vendor_name +")";
        }
        return repo.text;
    }
    $("#bank_code").select2({
