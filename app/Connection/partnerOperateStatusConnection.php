...
class partnerOperateStatusConnection
{
    public static function getList(array $params)
    {
        $url = env('API_URL_GATE_SERVICE') . '/api/v1/partner-operate-status';
    ...
    {
        $url = env('API_URL_GATE_SERVICE') . '/api/v1/partner-operate-status/create';
    ...
    {
        $url = env('API_URL_GATE_SERVICE') . '/api/v1/partner-operate-status/delete';
        $params = [
            'id'=> $id
        ];
        $result = Connection::sendRequest($url, $params, 'POST', AuthenticationHelper::getHeader());
    ...
    {
        $url = env('API_URL_GATE_SERVICE') . '/api/v1/partner-operate-status/update';
        $params['id'] = $id;
        $result = Connection::sendRequest($url, $params, 'POST', AuthenticationHelper::getHeader());
        $resultBody = json_decode($result['body']);
        if ($result['status_code'] != 200) {
