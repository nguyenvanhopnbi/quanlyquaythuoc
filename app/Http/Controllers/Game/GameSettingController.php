...
class GameSettingController extends Controller
{
    ...
    public function settings(Request $request)
    {
        ...
        if (!$request->request_type) {
            $statuses[''] = 'Tất cả';
            $statuses = array_merge($statuses, GameSettingTransformer::getStatuses());
            $actives[''] = 'Tất cả';
            $actives = array_merge($actives, GameSettingTransformer::getActives());
            $data = [
                'actives' => $actives,
                'statuses' => $statuses,
                'filter' => $filter,
            ];
            return view('game.setting', $data);
        } elseif ($request->request_type === 'download') {
            return \Response::download(public_path('/media/exports/') . $request->file)->deleteFileAfterSend(true);
        }
        ...
        if ($request->request_type === 'export') {
            $objExport = new ExportGamePaymentSetting($filter, $applications);
            $name = 'Danh sách giao dịch_' . now()->format('dmYHis') . '.xlsx';
            try {
                \Excel::store($objExport, $name, 'exports');
                event(new \App\Events\ActivityOccur(\App\Enums\LogCategoryEnum::GAME_EXPORT_SETTING, "Xuất file danh sách game", compact('filter')));
            } catch (\Exception $ex) {
                Log::error('+++Error: export GameController@settings ', [$ex]);
                return response(['code' => 500, 'message' => 'error'])->header('Content-Type', 'json');
            }
            return response(['code' => 200, 'message' => 'success', 'path' => $name])->header('Content-Type', 'json');
        }
        $overview = $this->gameService->settings($filter);
        $items = $overview['data'] ?? [];
        foreach ($items as &$item) {
            $item = GameSettingTransformer::convertSettingAttributes($item, $applications);
        }
        $paginate = [
            'data' => $items,
            'meta' => [
    ...
    public function settingApprove($id, Request $request)
    {
        $status = $request->status;
        $res = $this->gameService->settingApprove($id, $status);
        if($res['success']) {
            event(new \App\Events\ActivityOccur(\App\Enums\LogCategoryEnum::GAME_APPROVE, "Xét duyệt nạp game", compact('status')));
        }
        return response($res);
    }
    public function settingActive($id, Request $request)
    {
        $active = $request->active;
        $res = $this->gameService->settingActive($id, $active);
        if($res['success']) {
            event(new \App\Events\ActivityOccur(\App\Enums\LogCategoryEnum::GAME_ACTIVE, "Kích hoạt nạp game", compact('active')));
        }
        return response($res);
    }
    ...
    {
        $data = $request->all();
        $res = $this->gameService->settingUpdate($id, $data);
        if($res['success']) {
            event(new ActivityOccur(LogCategoryEnum::GAME_ACTIVE, "Cập nhật nạp game", compact('data')));
            return response($res);
        }
