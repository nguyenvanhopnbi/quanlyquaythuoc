...
class ExportShopCardPartnerData extends Controller
{
    public function ExportShopCardPartnerCardData(Request $request){
        $params = $request->all();
        ...
        $data = $ShopcardPartnerCardDataService->getList($params);
        ...
        if($pages >= 1){
            for ($i=$page; $i <= $pages ; $i++) {
                ...
                $data = $ShopcardPartnerCardDataService->getList($params)->data;
                $data = ShopcardPartnerCardDataTransformer::transformCollectionExport($data);
                // dd($data);
